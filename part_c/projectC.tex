...
\begin{tikzpicture}[auto]
	\node (KundeView) [abstract, rectangle split, rectangle split parts=1, text width=2cm]
        {
            \textbf{\small{KundeView}}
    	};
    	\node (LieferwegView) [abstract, rectangle split, rectangle split parts=1, below =0.5cm of KundeView, text width=3cm]
        {
            \textbf{\small{LieferwegView}}
    	};
	\node (Detailview) [abstract, rectangle split, rectangle split parts=1, text width=2cm, left = 0.5cm of LieferwegView]
        {
            \textbf{\small{DetailView}}
    	};
   \node (Listview) [abstract, rectangle split, rectangle split parts=1, below =6cm of Detailview, text width=2cm]
        {
            \textbf{\small{ListView}}
    	};
   	%Views    
   	\node (BestellungView) [abstract, rectangle split, rectangle split parts=1, below =3.5cm of LieferwegView, text width=3cm]
        {
	...
	\node (Kartendienst) [abstract, rectangle split, rectangle split parts=3, right=of LieferwegView]
        {
   ...
   \node (Model) [abstract, rectangle split, rectangle split parts=1, right =1cm of Pizza, text width=4cm]
        {
            \textbf{\small{django.db.models.Model}}
        };
    \path[-,line width=1pt] 	(Kartendienst) edge [bend left=0] node {$ $} (Lieferweg);
    \path[-,line width=1pt] 	(Kunde) edge [bend left=0] node {\small{* \tiny{Veranlasser} 1}} (Bestellung);
    \path[>=open diamond ,<-,line width=1pt] 	(Bestellung) edge [bend right=0] node {\tiny{* Pizzaliste 1}} (Pizza);
    \path[>=open diamond ,<-,line width=1pt] 	(ZutatMenge) edge [bend left=0] node {\tiny{1 Lebensmittel *}} (Zutat);
    \path[-,line width=1pt] 	(Pizza) edge [bend left=0] node {\small{* \tiny{Art} 2}} (Rezept);
    \path[-,line width=1pt] 	(Pizza) edge [bend left=0] node {\small{* \tiny{Toppings} 0...5}} (Zutat);
    \path[-,line width=1pt] 	(Rezept) edge [bend left=0] node {\small{* \tiny{Zutaten} 1...*}} (ZutatMenge);
    \path[-,line width=1pt] 	(Filiale) edge [bend left=0] node {\small{* \tiny{Bestand} 0...*}} (ZutatMenge);
    %Erben von Model
